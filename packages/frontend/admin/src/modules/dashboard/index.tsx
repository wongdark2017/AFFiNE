...
function TrendChart({
  ...
  if (points.length === 0) {
    return <div className="text-sm text-muted-foreground">{t('No data')}</div>;
  }
  ...
  if (points.length === 0 || visibleSeries.length === 0) {
    return (
      <div className="flex h-44 items-center justify-center rounded-lg border border-dashed border-border/60 bg-muted/15 text-sm text-muted-foreground">
        {t('No mail deliveries in this window')}
      </div>
    );
  }
...
function LicensePreviewDialog({
  ...
  const rows = license
    ? [
        [t('Status'), license.valid ? t('Valid') : t('Invalid')],
        [t('License ID'), license.id],
        [t('Workspace ID'), license.workspaceId],
        [t('Plan'), license.plan],
        [t('Recurring'), license.recurring],
        [t('Seats'), intFormatter.format(license.quantity)],
        [t('Issued At'), formatDateTime(license.issuedAt)],
        [t('File Expires At'), formatDateTime(license.expiresAt)],
        [t('License Ends At'), formatDateTime(license.endAt)],
        [t('Entity'), license.entity],
        [t('Issuer'), license.issuer],
      ]
    : [];
  ...
  return (
    <Dialog open={!!license} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>{t('License Preview')}</DialogTitle>
          <DialogDescription>
            {t('Signature and payload format are valid.')}
          </DialogDescription>
        </DialogHeader>
...
function DashboardActions({
  ...
  const notifyNoFileSelected = useCallback(() => {
    toast.error(t('No license file selected.'));
  }, []);

  const openLicensePicker = useCallback(() => {
    const input = inputRef.current;
    if (!input) {
      toast.error(t('Failed to open license file picker.'));
      return;
    }
  ...
  const handleLicenseFileChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      previewLicense({ license: file })
        ...
        .catch(error => {
          console.error(error);
          toast.error(t('Failed to preview license.'));
        });
    },
    ...
    <>
      <div className="flex flex-wrap items-center justify-end gap-3">
        <span className="text-xs text-muted-foreground tabular-nums">
          {t('Updated at {time}', { time: formatDateTime(updatedAt) })}
        </span>
    ...
    <Card className="border-border/60 bg-card shadow-1">
      <CardHeader className="flex flex-col gap-3 pb-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1.5">
          <CardTitle className="text-base">{t('Top Shared Links')}</CardTitle>
          <CardDescription>
            {t('Top {count} links in the last {days} days', {
              count: topSharedLinks.length,
              days: topSharedLinksWindow.effectiveSize,
            })}
          </CardDescription>
        </div>
        <PanelRangeSelect
          ariaLabel={t('Top shared links range')}
        ...
        {topSharedLinks.length === 0 ? (
          <div className="rounded-xl border border-dashed border-border/60 p-8 text-center bg-muted/15">
            <div className="text-sm font-medium">
              {t('No shared links in this window')}
            </div>
            ...
            <Button asChild variant="outline" size="sm" className="mt-4">
              <Link to={ROUTES.admin.workspaces}>{t('Go to Workspaces')}</Link>
            </Button>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">{t('Guest')}</TableHead>
                <TableHead>{t('Last Accessed')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
  ...
  return (
    <Card className="border-border/60 bg-card shadow-1">
      <CardHeader className="flex flex-col gap-3 pb-4 md:flex-row md:items-start md:justify-between">
        <div className="space-y-1.5">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            {t('Email Delivery Trend')}
          </CardTitle>
          <CardDescription>
            {analytics.window.bucket === 'Hour'
              ? t('{window} at hour bucket in UTC', {
                  window: mailWindowLabel(hours),
                })
              : t('{window} at day bucket in UTC', {
                  window: mailWindowLabel(hours),
                })}
          </CardDescription>
        </div>
        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row">
          <PanelRangeSelect
            ariaLabel={t('Email delivery range')}
            value={hours}
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="type">{t('Mail type')}</SelectItem>
              <SelectItem value="outcome">{t('Success / failure')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <MultiTrendChart
          ariaLabel={t('Email delivery trend')}
          points={chartPoints}
          series={chartSeries}
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
          <div className="h-full min-w-0 lg:col-span-5">
            <PrimaryMetricCard
              ...
              description={t('{hours}h active window', {
                hours: dashboard.syncWindow.effectiveSize,
              })}
            />
          </div>
          <div className="h-full min-w-0 lg:col-span-3">
            <SecondaryMetricCard
              title={t('Copilot Conversations')}
              value={intFormatter.format(dashboard.copilotConversations)}
              description={t('{days}d aggregation', {
                days: dashboard.copilotWindow.effectiveSize,
              })}
              icon={
              ...
              action={
                <PanelRangeSelect
                  ariaLabel={t('Copilot conversations range')}
            ...
            <Card className="h-full border-border/60 bg-card shadow-1">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {t('Workspace {workspace} • Blob {blob}', {
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <Card className="border-border/60 bg-card shadow-1 lg:col-span-1">
            <CardHeader className="flex flex-col gap-3 pb-4 md:flex-row md:items-start md:justify-between">
              <div className="space-y-1.5">
                <CardTitle className="text-base">
                  {t('Sync Active Users Trend')}
                </CardTitle>
                <CardDescription>
                  {t('{hours}h at minute bucket', {
                    hours: dashboard.syncWindow.effectiveSize,
                  })}
                </CardDescription>
              </div>
              <PanelRangeSelect
                ariaLabel={t('Sync active users range')}
            ...
            <CardContent className="space-y-3">
              <TrendChart
                ariaLabel={t('Sync active users trend')}
                points={syncPoints}
                primaryLabel={t('Sync Active Users')}
          ...
          <Card className="border-border/60 bg-card shadow-1 lg:col-span-2">
            <CardHeader className="flex flex-col gap-3 pb-4 md:flex-row md:items-start md:justify-between">
              <div className="space-y-1.5">
                <CardTitle className="text-base">
                  {t('Storage Trend (Workspace + Blob)')}
                </CardTitle>
                <CardDescription>
                  {t('{days}d at day bucket', {
                    days: dashboard.storageWindow.effectiveSize,
                  })}
                </CardDescription>
              </div>
              <PanelRangeSelect
                ariaLabel={t('Storage trend range')}
            ...
            <CardContent className="space-y-4">
              <TrendChart
                ariaLabel={t('Workspace and blob storage trend')}
                points={storagePoints}
                primaryLabel={t('Workspace Storage')}
                primaryFormatter={value => formatBytes(value)}
                secondaryLabel={t('Blob Storage')}
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {t('Blob')}: {formatBytes(dashboard.blobStorageBytes)}
                </div>
              </div>
            </CardContent>
          </Card>
